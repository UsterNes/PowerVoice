...
CTrayIcon::CTrayIcon(UINT uID, int nItemID)
{
...
LRESULT CTrayIcon::OnTrayNotify(WPARAM wID, LPARAM lEvent)
{
	...
	if (lEvent==WM_RBUTTONUP) {
		...
		::SetMenuDefaultItem(pSubMenu->m_hMenu, m_nDefaultMenuItem, TRUE);
		...
		::SendMessage(m_nid.hWnd, WM_COMMAND, pSubMenu->GetMenuItemID(m_nDefaultMenuItem), 0);
